...
- `scripts/opencode-pf.sh`: CLI de perfis isolados do OpenCode (`create/list/show/run/clone/remove/doctor`) — `run` usa `XDG_DATA_HOME` por perfil + `--standalone` (substitui o `opencode-multi`, que não isola no opencode v2). Launcher instalado em `~/.local/bin/opencode-pf`.
