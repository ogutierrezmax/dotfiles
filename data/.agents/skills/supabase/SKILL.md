---
name: supabase
description: 'Use ao realizar QUALQUER tarefa envolvendo Supabase. Gatilhos: produtos
  Supabase (Database, Auth, Edge Functions, Realtime, Storage, Vectors, Cron, Queues);
  bibliotecas de cliente e integrações SSR (supabase-js, @supabase/ssr) em Next.js,
  React, SvelteKit, Astro, Remix; problemas de autenticação (login, logout, sessões,
  JWT, cookies, getSession, getUser, getClaims, RLS); Supabase CLI ou servidor MCP;
  mudanças de esquema, migrações, auditorias de segurança, extensões Postgres (pg_graphql,
  pg_cron, pg_vector).'
metadata:
  author: supabase
  version: 0.1.0
