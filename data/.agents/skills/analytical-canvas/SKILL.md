...
description: O Cursor Canvas é um aplicativo React ao vivo que pode ser aberto ao
  lado do chat. Você DEVE usar um canvas quando o agente produzir um artefato analítico
  independente — análises quantitativas, investigações de faturamento, auditorias
  de segurança, revisões de arquitetura, conteúdo denso de dados, cronogramas, gráficos,
  tabelas, explorações interativas, ferramentas repetíveis ou qualquer resposta que
  se beneficie de um layout visual. Prefira um canvas ao apresentar resultados de
  ferramentas MCP (Datadog, Databricks, Linear, Sentry, Slack, etc.) onde os dados
  são o entregável — renderize em um canvas rico em vez de despejá-los em uma tabela
  markdown ou bloco de código. Se você se pegar prestes a escrever uma tabela markdown,
  pare e use um canvas. Você também DEVE ler esta skill sempre que criar, editar ou
  depurar qualquer arquivo .canvas.tsx.
metadata:
  surfaces:
  - ide
---
