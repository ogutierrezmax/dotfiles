...
description: Visualiza e modifica as configurações do Cursor CLI em ~/.cursor/cli-config.json.
  Use quando o usuário quiser alterar as configurações do CLI, configurar permissões,
  alternar modo de aprovação, ativar modo vim, alternar opções de exibição, configurar
  sandbox ou gerenciar qualquer preferência do CLI.
metadata:
  surfaces:
  - cli
---
